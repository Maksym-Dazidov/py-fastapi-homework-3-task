...
async def login(user: UserRegistrationSchema, db: AsyncSession = Depends(get_db),
    ...
    db_user = await db.scalar(select(UserModel).where(UserModel.email == user.email))
    if not db_user or not db_user.is_active or not verify_password(login_data.password, user.hashed_password):
        raise HTTPException(status_code=400, detail="Invalid email or token.")
    try:
        ...
        db_refresh_token = RefreshTokenModel(token=refresh_token, user_id=db_user.id, days_valid=7)
